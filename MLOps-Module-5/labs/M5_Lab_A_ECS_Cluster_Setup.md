...
> **🪟 Git Bash on Windows — read this once before you start.** MSYS (Git Bash's POSIX layer) auto-translates command-line arguments that start with `/` into Windows paths. So `--log-group-name /ecs/truck-delay-service` gets silently rewritten to `C:/Program Files/Git/ecs/truck-delay-service` and AWS rejects it. **Quotes don't help.** Three workarounds, pick one:
> - **Prefix every affected command with `MSYS_NO_PATHCONV=1`** — e.g. `MSYS_NO_PATHCONV=1 aws logs create-log-group --log-group-name /ecs/...`. Most surgical.
> - **Double the leading slash** (`--log-group-name //ecs/...`) — MSYS leaves `//`-prefixed strings alone.
> - **Run from PowerShell instead of Git Bash** — no path mangling. (You'll still need to translate the bash here-docs.)
>
> Affected args in this lab: `--log-group-name /ecs/...` (Step 2.5, Step 7) and (in Lab B) `--health-check-path /_stcore/health`. macOS / Linux / WSL users — ignore this callout entirely.
...
Priya's team has been running the Streamlit dashboard on the M3 EC2 instance, started by a systemd unit so it auto-restarts on reboot and survives SSH disconnects. That's a working *single-machine* deployment — and it's where it stops being good enough. The EC2 is in one AZ, so an AZ outage takes the dashboard down. There's no load balancer in front, so to ship a new version Priya has to SSH in, `git pull`, restart the systemd unit, and the dashboard is down for the ~10 seconds Streamlit takes to come back up. There's no health check that notices a hung Python process — last month a stale TCP socket left the dashboard "running" but returning blank pages for 4 hours before ops staff complained at 2 PM.

Priya wants the dashboard to behave like a real service: **multi-AZ failover** if a zone goes down, **zero-downtime rolling deploys** when a new image is pushed, **health-aware routing** that pulls dead replicas out of rotation automatically, and **horizontal scaling** when traffic spikes. systemd-on-EC2 can do exactly one of those (auto-restart on the same box) — the other three need a different architecture. In M5 you'll move the container from "managed by systemd on a single EC2 I SSH'd into" to "managed by ECS as a service behind an Application Load Balancer". Today's lab is the foundation — get the container running on ECS Fargate. Lab B adds the public URL and the health-check routing.

---

## About ECS (Elastic Container Service) — first encounter

### What it is

**ECS is AWS's container orchestrator.** You hand it a container image and a JSON "recipe" (a *task definition*) that says "use this much CPU/memory, expose this port, send logs here". ECS schedules containers onto compute, keeps them running, restarts crashed ones, and does rolling updates when you change the recipe.

Three vocabulary words you must know cold:

| Word | What it is | Mental model |
|---|---|---|
| **Cluster** | A logical namespace for tasks + services | A folder. Holds zero or many services. Free to create. |
| **Task definition** | A JSON document: image URI, CPU, memory, ports, env vars, IAM role | A *recipe* — versioned (`truck-delay-task:1`, `:2`, ...) |
| **Service** | A long-running supervisor: "always keep N copies of this task running" | A *supervisor process* that restarts failed tasks + does rolling deploys |

The relationship: a **service** runs N copies of a **task definition** inside a **cluster**.

### The problem ECS solves (specifically for this project)

Without ECS, deploying the M4 Truck Delay container to AWS means: provision an EC2, SSH in, install Docker, `docker run` the image, write a systemd unit to auto-restart, configure log shipping, and write your own scripts for rolling updates. That's ~200 lines of bash and a fragile chain. ECS replaces all of it with: register a task definition, create a service.

What ECS handles for you in M5:

1. **Pulls the image from ECR** automatically when a task starts (uses the `ecsTaskExecutionRole` you'll create in Step 2)
2. **Restarts crashed tasks** without intervention (the service's job)
3. **Forwards container logs to CloudWatch Logs** via the `awslogs` log driver — one task-definition field, no log-shipping agent
4. **Does rolling deploys** when the task definition changes (Lab C wires `git push` → new revision)
5. **Distributes tasks across Availability Zones** automatically when desired-count ≥ 2 (we'll see this in Lab B)

### Fargate vs ECS-on-EC2 — the launch-type choice

ECS has **two launch types** for the compute that runs your containers. Picking the right one is the single most important architectural decision in this lab.

| Aspect | **Fargate** (we're using this) | **ECS-on-EC2** |
|---|---|---|
| Who manages the compute | AWS — invisible, per-task | You — provision + patch + autoscale a fleet of EC2 instances |
| Billing model | Pay per-second per-vCPU + per-GB-memory of running tasks | Pay per-EC2-hour (whether tasks are using the instance or not) |
| Cold start | ~10-30 sec per task launch | <5 sec if the EC2 is warm |
| GPU support | Limited (only specific Fargate types) | Full — any EC2 GPU instance |
| When it wins | Bursty traffic, small fleets, "I don't want to manage EC2" | Sustained high utilisation (≥70%), GPU workloads, custom kernels |

**For Truck Delay (Streamlit, low-to-medium QPS, bursty), Fargate is clearly right.** We pay for what we use, AWS handles the underlying OS + patching, and the per-task billing means scaling to zero literally costs ₹0.

### Pricing (ap-south-1, Fargate Linux/x86)

| Component | Rate | Cost for our task (0.5 vCPU, 1 GB) |
|---|---|---|
| vCPU | ~$0.04048 / vCPU-hour | 0.5 vCPU × ~₹3.4/hour = **~₹1.7/hour** |
| Memory | ~$0.00445 / GB-hour | 1 GB × ~₹0.37/hour = **~₹0.4/hour** |
| **Total per task** | | **~₹2/hour** while running |
| Per-second granularity | Min charge = 1 minute, then per-second | Tear-down savings are real |
| Storage (ephemeral, first 20 GB) | Free | ₹0 |
| Egress to internet | Standard data transfer pricing | Negligible for this lab |

**For a 4-hour class session**: 1 task × ₹2/hour × 4 hours = **~₹8**. Cheap enough to leave running through Labs A→B→C; tear down at end of session.

### How ECS compares to what we've already used

| Tool | Where you saw it | Why ECS replaces it for production |
|---|---|---|
| **`docker run` on a laptop** (M4 Lab 2) | Quickest way to start a container | Stops when the laptop sleeps; one machine; no health checks |
| **`docker compose up`** (M4 Lab 3) | Multi-container local stack | Single machine; not network-addressable from outside; no auto-restart across reboots without extra config |
| **`docker push` to ECR** (M4 Lab 4) | Stored the image in AWS | ECR is *storage*, not *runtime* — pushing an image to ECR doesn't run it anywhere |
| **systemd on M3 EC2** | Auto-restart on the same box | Single-machine; single-AZ; no rolling updates; no health-aware routing |
| **ECS (this lab)** | Production-grade container runtime | Multi-AZ, auto-restart, rolling deploys, health-aware (with the ALB from Lab B) |

The progression is: image stored (ECR, M4) → image runs as a managed service (ECS, this lab) → image is publicly addressable + load-balanced (ALB, Lab B) → image auto-redeploys on commit (CI/CD, Labs C/D).
...
> **🪟 Windows users:** two compatibility gotchas combined here. (1) The bash here-doc below (`cat > file <<'EOF'`) is not valid PowerShell — use Git Bash / WSL for the here-doc, or save the JSON manually to `ecs-trust-policy.json` with Notepad / VS Code. (2) The original lab used `/tmp/...` — **that path doesn't work on Windows** even from Git Bash, because the native Windows AWS CLI can't see Git Bash's emulated `/tmp`. We now write to a relative path (`./ecs-trust-policy.json`) which works on every shell.

```bash
# Trust policy: who can assume this role (only ECS tasks)
# Writes to the current directory (works on macOS / Linux / Git Bash / WSL).
...
## Step 2.5: Pre-create the CloudWatch Log Group

> **🚨 Why this step exists:** the AWS-managed `AmazonECSTaskExecutionRolePolicy` you just attached covers `logs:CreateLogStream` and `logs:PutLogEvents`, but **NOT** `logs:CreateLogGroup`. If you point the task at a log group that doesn't exist yet (which is the lab's default), ECS will fail every task launch with `AccessDeniedException` and retry forever. Pre-creating the log group sidesteps the problem entirely.
...
# Optional but recommended -- keep 7 days of logs, not 'never expire' (the default)
...
> **🪟 Windows Git Bash users**: arguments starting with `/` get mangled into Windows paths by MSYS. Prefix every command above with `MSYS_NO_PATHCONV=1` — e.g. `MSYS_NO_PATHCONV=1 aws logs create-log-group ...`. Or use PowerShell where this isn't an issue. (See top-of-lab callout for the full pattern.)

Verify:
```bash
aws logs describe-log-groups \
    --log-group-name-prefix /ecs/truck-delay-service \
    --region ap-south-1 \
    --query "logGroups[].{Name:logGroupName, Retention:retentionInDays}" --output table
```

> **The alternative — granting `logs:CreateLogGroup` on the execution role + keeping `awslogs-create-group: true` in the task definition** — works too, but it (a) departs from "infrastructure is pre-provisioned, not auto-created at runtime", and (b) hides the failure mode from learners who would otherwise debug "task stuck pending" for 20 minutes. We pre-create here so the lab fails loud and early if something else goes wrong.
...
| `logConfiguration.awslogs-group` | The CloudWatch log group ECS writes container logs to. **Must already exist** — we pre-created it in Step 2.5. (The `awslogs-create-group: "true"` option exists but requires the execution role to have `logs:CreateLogGroup`, which the AWS-managed policy doesn't grant.) |
...
| Service events show `AccessDeniedException: User ... is not authorized to perform: logs:CreateLogGroup` (tasks fail to start, retry forever) | The AWS-managed `AmazonECSTaskExecutionRolePolicy` doesn't include `logs:CreateLogGroup` — so `awslogs-create-group: "true"` in a task-def will always fail | Don't use `awslogs-create-group: "true"`. Pre-create the log group via `aws logs create-log-group --log-group-name /ecs/truck-delay-service` (Step 2.5). Then force-redeploy: `aws ecs update-service ... --force-new-deployment` |
...
> **🪟 Git Bash on Windows users:** prefix every `aws logs ...` command below with `MSYS_NO_PATHCONV=1` — `/ecs/...` gets path-mangled otherwise. (See top-of-lab callout.)

```bash
# Cluster
aws ecs create-cluster --cluster-name m5-truck-delay-cluster --region ap-south-1

# Pre-create the log group (Step 2.5 -- required because the managed execution role
# policy doesn't include logs:CreateLogGroup)
aws logs create-log-group --log-group-name /ecs/truck-delay-service --region ap-south-1
aws logs put-retention-policy --log-group-name /ecs/truck-delay-service --retention-in-days 7 --region ap-south-1
