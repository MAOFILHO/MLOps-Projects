...
4. **Create topic**. Note the **ARN** that appears on the success page — looks like `arn:aws:sns:us-east-1:<ACCOUNT_ID>:truck-delay-alerts`. You'll paste this everywhere.
...
Output:
```json
{ "TopicArn": "arn:aws:sns:us-east-1:<ACCOUNT_ID>:truck-delay-alerts" }
```

Save the ARN as a shell variable for later steps:
```bash
export TOPIC_ARN=arn:aws:sns:us-east-1:<ACCOUNT_ID>:truck-delay-alerts
```
...
```bash
# 🪟 Windows PowerShell: $env:TOPIC_ARN = "arn:aws:sns:us-east-1:<ACCOUNT_ID>:truck-delay-alerts"
# Linux/macOS/Git Bash:  export TOPIC_ARN=arn:aws:sns:us-east-1:<ACCOUNT_ID>:truck-delay-alerts
...
- [ ] `aws sns list-topics --region us-east-1` shows `truck-delay-alerts`
...
| `python sns_publish_test.py` errors `botocore.exceptions.NoRegionError` | `AWS_REGION` not set in environment | `export AWS_REGION=us-east-1` or pass `region_name=` to `boto3.client(...)` |
...
```bash
# Topic
aws sns create-topic --name truck-delay-alerts --region us-east-1
export TOPIC_ARN=arn:aws:sns:us-east-1:<ACCOUNT_ID>:truck-delay-alerts
