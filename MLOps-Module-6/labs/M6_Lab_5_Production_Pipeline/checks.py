...
def run_drift_detection(reference: pd.DataFrame, current: pd.DataFrame) -> tuple:
    ...
    all_results = [m["result"] for m in report.as_dict()["metrics"]]
    summary = next(res for res in all_results if "drift_share" in res)
    table = next(res for res in all_results if "drift_by_columns" in res)
    ...
    details = {
        ...
        "drifted_columns": [c for c, cr in table["drift_by_columns"].items() if cr["drift_detected"]][:10],
    }
