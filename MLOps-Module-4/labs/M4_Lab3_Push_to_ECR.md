...
{
    "repository": {
        "repositoryArn": "arn:aws:ecr:us-east-1:123456789012:repository/truck-delay-app",
        "repositoryUri": "123456789012.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app",
...
```
<ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app
```

Now open the AWS Console in your browser, navigate to **ECR** (search for "Elastic Container Registry"), and confirm the repository appears in the Mumbai (us-east-1) region.
...
>
> ```bash
> aws ecr put-lifecycle-policy --repository-name truck-delay-app --region us-east-1 \
...
```bash
docker tag truck-delay-app:v1 <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app:v1
```
...
```bash
docker tag truck-delay-app:v1 123456789012.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app:v1
```
...
```bash
docker push <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app:v1
```

Expected output:

```
The push refers to repository [123456789012.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app]
5a3b8c2d1e4f: Pushed
7f6e5d4c3b2a: Pushed
...
...
5. **Pull and run the image:**
    ...
    ```bash
    docker pull <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app:v1
    docker run -d -p 8501:8501 -e DEMO_MODE=true --name truck-app <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app:v1
    ```
...
```bash
docker rmi <ACCOUNT_ID>.dkr.ecr.us-east-1.amazonaws.com/truck-delay-app:v1
```
...
| ECR data transfer (pull) | Free within same region | $0 if EC2 and ECR are both in us-east-1 |
| ECR data transfer (cross-region) | $0.09 per GB | Avoid by keeping everything in us-east-1 |
...
- [ ] `aws ecr describe-repositories --region us-east-1` lists `truck-delay-app`.
