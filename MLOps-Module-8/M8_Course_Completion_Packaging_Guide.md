...
> SageMaker Pipeline triggered by EventBridge (M8) — all in one AWS account, us-east-1.*
...
The cost incidents in this course came from **forgetting to tear down**. Do this in order. Region: **us-east-1**
...
aws scheduler delete-schedule --name truck-delay-weekly-retrain --region us-east-1 2>/dev/null || true
...
### B.4 Lambda
```bash
aws lambda delete-function --function-name truck-delay-land-streaming --region us-east-1 2>/dev/null || true
```

### B.5 The SageMaker notebook (if not removed by B.2)
```bash
aws sagemaker stop-notebook-instance --notebook-instance-name m6-truck-delay-monitoring --region us-east-1 2>/dev/null || true
# wait for Stopped, then:
aws sagemaker delete-notebook-instance --notebook-instance-name m6-truck-delay-monitoring --region us-east-1 2>/dev/null || true
```
...
aws s3 ls | grep sagemaker-$R   # sagemaker-us-east-1-<account>
# aws s3 rb s3://sagemaker-us-east-1-<account> --force   # only if you don't reuse it
```
