...
| What | Why | Where |
|---|---|---|
| **SageMaker notebook from M6/M7** | author + run the pipeline (needs `get_execution_role()`) | `m6-truck-delay-monitoring` (same VPC, us-east-1) |
...
export PIPELINE_BUCKET=$(aws cloudformation describe-stacks --stack-name m8-stack \
  --query "Stacks[0].Outputs[?OutputKey=='ArtifactBucket'].OutputValue" --output text --region us-east-1)
export PIPELINE_TOPIC_ARN=$(aws cloudformation describe-stacks --stack-name m8-stack \
  --query "Stacks[0].Outputs[?OutputKey=='TopicArn'].OutputValue" --output text --region us-east-1)
...
> Set `PIPELINE_BUCKET`, `ACCOUNT`, and `REGION=us-east-1` first (see [§1](#1-prerequisites--environment)).
...
aws sagemaker create-model-package-group --model-package-group-name TruckDelayModelPackageGroup \
  --model-package-group-description "Truck Delay capstone registry" --region us-east-1

# Upload the training data
aws s3 cp labs/data/reference/final_features.csv \
  s3://$PIPELINE_BUCKET/truck-delay/input/final_features.csv --region us-east-1
...
aws sagemaker start-pipeline-execution --pipeline-name TruckDelayClassification \
  --pipeline-parameters Name=F1Threshold,Value=0.55 --region us-east-1
aws sagemaker list-pipeline-execution-steps --pipeline-execution-arn <arn> --region us-east-1
...
aws sagemaker list-model-packages --model-package-group-name TruckDelayModelPackageGroup \
  --sort-by CreationTime --sort-order Descending --region us-east-1
aws sagemaker update-model-package --model-package-arn <arn> --model-approval-status Approved --region us-east-1
```
...
aws lambda invoke --function-name truck-delay-land-streaming --payload '{}' \
  --cli-binary-format raw-in-base64-out out.json --region us-east-1 && cat out.json
```
...
**Teardown:**
```bash
aws scheduler delete-schedule --name truck-delay-weekly-retrain --region us-east-1
aws lambda delete-function --function-name truck-delay-land-streaming --region us-east-1
aws sagemaker delete-pipeline --pipeline-name TruckDelayClassification --region us-east-1
